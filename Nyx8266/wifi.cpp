...
namespace wifi {
    ...
    void connectHandler(){
        if (server.hasArg("ssid")) {
            ...
            /* removed delay(100) — any delay() blocks the entire main loop,
               stalling attacks, scan, LED, and display update callbacks */
            ...
            String html = "<html><body><h2>Connect to upstream WiFi</h2>";
    ...
    bool saveWiFiEvil(String ssid, String pass){
        ...
        tmp += (!lastBuffer.isEmpty() ? lastBuffer + String(COMMA) : String("")) +
            String(OPEN_CURLY_BRACKET) +
                String(DOUBLEQUOTES) + "ssid" + String(DOUBLEQUOTES) + String(DOUBLEPOINT) + String(DOUBLEQUOTES) + escape(ssid) + String(DOUBLEQUOTES) + String(COMMA) +
                String(DOUBLEQUOTES) + "pass" + String(DOUBLEQUOTES) + String(DOUBLEPOINT) + String(DOUBLEQUOTES) + escape(pass) + String(DOUBLEQUOTES) +
    ...
    void startWebServer(){
        dns.stop(); // ensure clean restart if called more than once (e.g. after EvilTwin::stop)
        ...
        if (!settings::getWebSettings().use_spiffs) {
            ...
            /*
             * BUG FIX: ESP8266WebServer upload pattern.
             *
             * server.on("/upload", HTTP_POST, <finalCb>, <uploadCb>)
             *
             * The upload callback is called repeatedly as chunks arrive.
             * The final callback is called once after all chunks are received.
             *
             * Old code tried to read server.upload() inside the final callback
             * without an upload callback — the upload buffer was never populated,
             * causing every upload to silently fail or crash.
             *
             * New code opens the LittleFS file once (on UPLOAD_FILE_START),
             * streams each chunk directly to disk (UPLOAD_FILE_WRITE),
             * and closes it when done (UPLOAD_FILE_END).  This avoids
             * bufferToString() which caused O(N) heap reallocations — one per
             * byte — crashing the ESP8266 for files larger than ~4 KB.
             */
            server.on("/upload", HTTP_POST,
                /* final callback — send result after upload completes */
                []() {
                    server.send(200, "text/html",
                        "<html><body><script>alert('Upload complete. Reloading...');"
                        "window.location.href='/fsmanager.html';</script></body></html>");
                },
                /* upload callback — called per chunk */
                []() {
                    ...
                    if (upload.status == UPLOAD_FILE_START) {
                        ...
                        /* remove existing file first to avoid stale data */
    ...
    void update() {
         ...
         *
         * The original code skipped server.handleClient() while a scan was
         * running, dropping every HTTP request the browser sent during that
         * window.  The browser's XHR timeout then fired, showing "✗ offline".
         *
         * The packet sniffer runs via a promiscuous-mode callback (interrupt-
         * driven), so it does NOT block the main loop.  The web server and the
         * scanner can safely run concurrently; there is no reason to suppress
         * HTTP handling during a scan.
         */
