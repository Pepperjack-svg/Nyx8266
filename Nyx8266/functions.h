...
bool readFile(String path, String& buf) {
    ...
    /* Pre-allocate prevents O(N²) realloc chain on char-by-char append */
    buf.reserve(buf.length() + sz);
...
bool writeFile(String path, String& buf) {
    ...
    File f = LittleFS.open(path, "w+");
    if (!f) return false;
    /* Bulk write: single syscall instead of len individual write() calls */
    f.write(reinterpret_cast<const uint8_t*>(buf.c_str()), buf.length());
    f.close();
    return true;
}
