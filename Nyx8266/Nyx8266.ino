/* =====================
   ...
   https://github.com/Pepperjack-svg/Nyx8266
   ===================== */
...
void loop() {
    ...
    if (resetButton->holding(3000)) {
        ...
        /* Non-blocking 2s display hold — feeds WDT and keeps WiFi stack alive */
        uint32_t _resetEnd = millis() + 2000;
        while (millis() < _resetEnd) {
            led::update();
            displayUI.update();
            wifi::update();
            yield();
