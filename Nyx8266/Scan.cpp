    ...
    if ((scanMode == SCAN_MODE_APS) || (scanMode == SCAN_MODE_ALL)) {
        ...
        if (results >= 0) {
            ...
            if (scanMode == SCAN_MODE_ALL) {
                /* delay(30) removed — it blocked the main loop and was meant
                   to let the SDK settle after WiFi.scanComplete().  The SDK
                   transition to promiscuous mode inside start(SCAN_MODE_STATIONS)
...
void Scan::nextChannel() {
    if (accesspoints.count() >= 1) {
        ...
        do {
            ch++;
            if (ch > 14) ch = 1;
            if (++tries > 14) return; // no valid channel found, avoid infinite loop
...
uint8_t Scan::getPercentage() {
    if (!isSniffing() || sniffTime < 100) return 0;

    uint32_t divisor = sniffTime / 100;
    if (divisor == 0) return 0;
    uint32_t elapsed = currentTime - snifferStartTime;
    uint32_t pct = elapsed / divisor;
