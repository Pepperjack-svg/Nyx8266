...
class Attack {
    ...
    private:
        ...
        struct AttackType {
            ...
            uint16_t maxPkts       = 0;
            uint16_t tc            = 0; // uint16_t: uint8_t wraps at 256, corrupting large SSID/AP lists
            uint32_t time          = 0;
        };
        ...
        /* Rolling sequence number for deauth/disassoc frames.
           Devices and APs deduplicate management frames by (BSSID, seqNum).
           Incrementing prevents silent drops on modern drivers/firmwares. */
        uint16_t seqNum = 0;

        /* Pre-allocated beacon build buffer.
           Max beacon size = 77 header bytes + 32 SSID bytes = 109.
           Avoids new[]/delete[] on every sendBeacon() call (up to 100/sec),
           which caused heap fragmentation and eventual crash. */
        uint8_t beaconBuf[109];
